...
Exemplars:
- [`VendorNPC.cs`](../../../Assets/_Project/Scripts/World/VendorNPC.cs) — stationary, barter trade, immortal
- [`CompanionNPC.cs`](../../../Assets/_Project/Scripts/World/CompanionNPC.cs) — follows player,
  FSM (R7), hunger decay + starvation damage, multi-component composition
...
|---|---|---|
| Vendor | `HealthComponent` + `InvulnerabilityComponent` (default on) | Immortal, barter trade. Exemplar: `VendorNPC` |
| Companion | `HealthComponent` + `HungerComponent` + `InvulnerabilityComponent` + `StateMachine<T>` | AI follow (Idle/Follow/Dead FSM) + hunger decay + starvation damage. Exemplar: `CompanionNPC` |
...
5. **AI (nếu AI-driven)**: dùng `StateMachine<T>` theo pattern R7. Xem `add-mob/` skill +
   `WolfStates.cs`. Companion FSM exemplar: `Scripts/World/States/CompanionStates.cs`
   (Idle / Follow / Dead — 3 state minimal scope, pattern mirror mob FSM).
   Host MonoBehaviour: `readonly StateMachine<MyNPC> Fsm = new();` → `Fsm.Init(this, States.Idle)`
   trong Awake → `Fsm.Tick(Time.deltaTime)` trong Update. Shutdown trong OnDisable nếu cần.
...
6. **EditMode test** (~10-15 case):
   ...
   - Skill exemplars:
    - [`VendorNPCTests.cs`](../../../Assets/_Project/Tests/EditMode/VendorNPCTests.cs) — stationary NPC
    - [`CompanionNPCTests.cs`](../../../Assets/_Project/Tests/EditMode/CompanionNPCTests.cs) — FSM + survival
